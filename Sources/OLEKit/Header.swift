...
extension OLEFile {
  ...
  public struct Header {
    ...
    init(_ stream: inout DataStream, fileSize: UInt64) throws {
      guard stream.read(count: magic.count) == magic else { throw OLEError.fileIsNotOLE }
      ...
      let miniFATSectorShift: UInt16 = stream.read()

      miniSectorSize = UInt16(pow(Double(2), Double(miniFATSectorShift)))
